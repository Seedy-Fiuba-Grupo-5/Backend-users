...
from prod.exceptions import BusinessError, InvalidTransitionAmount, UserBlockedError, InvalidTransitionType
...
from prod.schemas.constants import MISSING_VALUES_ERROR, REPEATED_USER_ERROR, INVALID_TOKEN
...
class UsersTransactionListResource(BaseResource):
    ...
    code_20x_swg = ns.model('All transactions input 20x', {
        ...
        'user_id': fields.Integer(description='The user id associated with the transaction'),
        'projects_id': fields.Integer(description='The project id associated with the transaction'),
    ...
    def post(self):
        ...
        try:
            ...
            if isinstance(token_decoded, str) or data['user_id'] != token_decoded:
            ...
            if owner_id == data['user_id'] and data['type'] == 'support':
                ns.abort(401, status='An Project Owner cannot support his/her own project')
            if owner_id != data['user_id'] and data['type'] == 'pay':
                ns.abort(401, status='Only Project Owner can make Pay transactions')
            user_model = TransactionsDBModel.add_transaction(data['user_id'],
                                      data['project_id'],
                                      data['amount'],
                                      data['type'])
            ...
            ns.abort(403, status='Transaction type must be either support or pay')
