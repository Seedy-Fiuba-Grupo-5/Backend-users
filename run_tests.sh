...
docker-compose exec service_users_web pytest "backend_users/dev/tests" --cov="backend_users/prod" -p no:warnings

echo "[RUN_TESTS] Running flake8 inside all folders"
docker-compose exec service_users_web flake8 backend_users/prod
